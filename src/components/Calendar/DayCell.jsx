...
export default function DayCell({ dateKey, day, count, onClick }) {
  ...
  return (
    <button onClick={onClick} aria-label={`${dateKey} 실행 ${count}개`} className={`rounded border border-gray-200 h-13 flex flex-col items-center justify-start p-1 text-sm  ${isToday ? 'bg-gray-500' : 'bg-gray-990'}`}>
      ...
      <div className='flex items-center justify-center gap-[-4px]'>
        ...
        {overflowCount > 0 && <span className='text-[10px] text-gray-200 ml-1'>+{overflowCount}</span>}
      </div>
